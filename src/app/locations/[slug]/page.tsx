...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = (loc as Record<string, unknown>).metaDescription as string | undefined ?? (loc as Record<string, unknown>).intro as string ?? "";
  return {
    title: (loc as Record<string, unknown>).metaTitle as string ?? `${SITE.name} — ${loc.name}`,
    description: desc,
    alternates: { canonical: url },
    openGraph: { title: `${SITE.name} — ${loc.name} | Contractors Choice Agency`, description: desc, url },
  };
}
...
export default async function LocationPage({ params }: Props) {
  ...
  const locIntro = (loc as Record<string, unknown>).intro as string ?? (loc as Record<string, unknown>).metaDescription as string ?? "";
  const serviceSchema = {
    "@context": "https://schema.org",
    "@type": "InsuranceService",
    name: `${SITE.name} — ${loc.name}`,
    description: locIntro,
    url,
  ...
  return (
    <>
      ...
      <main>
          ...
          <div className="container-wide">
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <FadeIn className="lg:col-span-7">
                ...
                <p className="mt-5 lead max-w-2xl">{locIntro}</p>
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Custom truck in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Coverage for {loc.name} truck owners</span>
              <h2 className="mt-3 h-section">The full program, built for {loc.name} custom trucks.</h2>
              <p className="mt-4 lead">From a single lifted daily driver to a full fleet of modified builds, we coordinate every line a {loc.name} custom truck owner needs.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>Custom truck insurance questions for <span className="text-clay">{loc.name}</span></>} background="sand" />

        <CTABand title={`Insuring custom trucks in ${loc.name} since 2005`} description={`Local knowledge, A-rated markets, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
